...
const TimesheetGrid = ({ rows, setRows, disabled, dailyTotals }) => {
  ...
  if (!rows.length) return <div className="timesheet-empty"><FiTrash2 /><strong>No time entries yet</strong><p>Add a recent project to begin this week.</p></div>;

  return <div className="timesheet-grid-wrap"><table className="timesheet-table">
    <thead><tr><th>Project & task</th>{days.map((day) => <th key={day} className={["sat", "sun"].includes(day) ? "weekend-col" : ""}>{day.slice(0, 1).toUpperCase() + day.slice(1)}</th>)}<th>Total</th>{!disabled && <th aria-label="Actions" />}</tr></thead>
    <tbody>{rows.map((row, index) => {
      const total = days.reduce((sum, day) => sum + Number(row[day] || 0), 0);
      return <tr key={row.id || `${row.project_id}-${row.task_name}`}>
        <td data-label="Work"><strong>{row.project_name}</strong><span>{row.task_name}</span><small>{row.worked_on}</small></td>
