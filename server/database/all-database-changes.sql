-- TeamFlow Manager - consolidated database changes
-- Generated from server/utils/dbMigrations.js.
-- Run this file against an existing TeamFlow Manager MySQL database.
-- It is ordered to create dependency tables before altering or populating them.
-- Existing application data is preserved.
SET @entry_type_exists = (
  SELECT COUNT(*)
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'weekly_timesheets'
    AND COLUMN_NAME = 'entry_type'
);
SET @entry_type_sql = IF(
  @entry_type_exists = 0,
  'ALTER TABLE weekly_timesheets ADD COLUMN entry_type ENUM(''project'', ''leave'', ''training'') NOT NULL DEFAULT ''project'' AFTER year',
  'SELECT 1'
);
PREPARE entry_type_statement FROM @entry_type_sql;
EXECUTE entry_type_statement;
DEALLOCATE PREPARE entry_type_statement;
...
ALTER TABLE users
      ADD COLUMN IF NOT EXISTS position_id INT NULL AFTER role,
      ADD COLUMN IF NOT EXISTS is_active TINYINT(1) NOT NULL DEFAULT 1 AFTER position_id;
